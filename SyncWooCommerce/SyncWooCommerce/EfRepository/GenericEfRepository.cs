...
namespace SyncWooCommerce.EfRepository
{
    /// <summary>
    ...
    public class GenericEfRepository<TEntity> : IGenericEfRepository<TEntity> where TEntity : class
    {
        ...
        public GenericEfRepository(ContextDB context)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public virtual IEnumerable<TEntity> GetPaginado<TKey>(Expression<Func<TEntity, TKey>> orderBy, int pagina, int qtdeItens)
        {
            return DbSet.OrderBy(orderBy).Skip((pagina - 1) * qtdeItens).Take(qtdeItens).ToList();
        }
        #endregion
    }
}
